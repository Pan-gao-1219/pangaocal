...
class MajorConfig:
    ...
    def __init__(self):
      ...
      # ============ 24勘工（卓越工程师班） ============
        self.major_24kg = {
            ...
            '卓越班级学号集': {
                # 卓越工程师名单
                '刘海波', '徐玉松', '汪小康', '苏柱仁', '贾驰航', '杨晨',
                '张丽君', '王官正', '胡焱斌', '曾宗荣', '周欣宇', '张继鹏',
                '郭城晔', '周扬', '马帅', '黄贺强', '柳坤', '彭喆',
                '石彦羿', '郑景珂', '崔茗芮', '李科甫', '曹莹', '候顺梦',
                '蒋林志', '格桑文修', '胡江涛', '李政贤'
            },
            '学分要求': {
                '卓越': {
    ...
    def get_major(self, major_code):
        ...
        elif major_code == '24kg':  # ← 新增
            return self.major_24kg
        else:
            return None
...
def main():
    ...
    col1, col2, col3, col4 = st.columns(4)  # ← 改成4列

    with col1:
        if st.button("📚 23勘工（有卓越班）", ...):
            ...

    with col2:
        if st.button("🗺️ 23地质（统一班级）", ...):
            ...

    with col3:
        if st.button("🛰️ 23地信（统一班级）", ...):
            ...

    with col4:  # ← 新增第4个按钮
        if st.button("⚙️ 24勘工（卓越工程师）", use_container_width=True,
                     type="primary" if st.session_state.major_code == '24kg' else "secondary"):
            st.session_state.major_code = '24kg'
            calc.set_major('24kg')
            st.session_state.calc = calc
            st.rerun()
